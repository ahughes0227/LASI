...
def test_projector_maps_known_confidence_labels() -> None:
    ...
    assert {
        fact.subject: fact.confidence for fact in snapshot.facts if fact.predicate == "statement"
    } == {
        "claim:low": 0.25,
        "claim:medium": 0.5,
        "claim:high": 0.8,
...
def test_projector_maps_unknown_confidence_to_zero() -> None:
    snapshot = project(
        ProjectState(
            ...
            claims=[
                EvidenceClaim(
                    claim_id="c1", project_id="p1", claim="x", confidence="certain", status="weak"
                )
            ],
        )
    )
...
def test_projector_output_is_deterministic_under_input_reordering() -> None:
    one = ProjectState(
        project_id="p1",
        experiments=[
            ExperimentContext(project_id="p1", experiment_id="b", hypothesis="b"),
            ExperimentContext(project_id="p1", experiment_id="a", hypothesis="a"),
        ],
        claims=[
            EvidenceClaim(
                claim_id="b", project_id="p1", claim="b", confidence="low", status="weak"
            ),
            EvidenceClaim(
                claim_id="a", project_id="p1", claim="a", confidence="low", status="weak"
            ),
        ],
    )
    two = one.model_copy(
        update={
            "experiments": list(reversed(one.experiments)),
            "claims": list(reversed(one.claims)),
        }
    )
    assert project(one).model_dump() == project(two).model_dump()
    assert [fact.fact_id for fact in project(one).facts] == sorted(
        fact.fact_id for fact in project(one).facts
    )
