...
def _contract(
    capability_id, *, preconditions=(), effects=(), side_effect_class="read", validations=()
):
    return CapabilityDomainContract(
        ...
        validation_capability_ids=list(validations),
...
def _snapshot(*facts):
    return DomainStateSnapshot(
        project_id="project-1", revision=4, facts=list(facts), created_at=NOW
    )
...
def _goal(*predicates, **kwargs):
    return DomainGoal(
        ...
        description="test",
        desired_state=[
            _predicate(*item) if isinstance(item, tuple) else item for item in predicates
        ],
        **kwargs,
    )
...
def test_approval_policy_is_preserved_in_plan():
    policy = DomainPolicy(
        ...
        reason="review",
        required_approval_action="change",
        provenance_refs=["test"],
    )
    planner = _setup(_contract("update", effects=[_effect("ready")]), policies=[policy])
    plan = planner.plan(_goal(("ready", True)), _snapshot())
...
def test_planner_uses_deterministic_tie_breaking():
    planner = _setup(
        _contract("z-read", effects=[_effect("ready")]),
        _contract("a-read", effects=[_effect("ready")]),
    )
    plan = planner.plan(_goal(("ready", True)), _snapshot())
    ...
    with pytest.raises(ValueError, match="project_id"):
        planner.plan(
            DomainGoal(
                ...
                project_id="other",
                description="x",
                desired_state=[_predicate("ready")],
            ),
            _snapshot(),
        )
