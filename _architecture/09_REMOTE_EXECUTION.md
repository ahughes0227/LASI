...
Every remote command carries a required wall-clock timeout with a four-hour
default, so a caller cannot request an unbounded remote run by omitting one. The
transport receives that bound directly; exceeding it produces the `timeout`
failure type rather than an indefinitely held run.
