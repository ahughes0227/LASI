...
class DomainStatePlanner:
    ...
    def plan(self, goal: DomainGoal, snapshot: DomainStateSnapshot) -> DomainPlan:
        ...
        if not unresolved:
            return self._make_plan(
                goal, snapshot, "satisfied", (), initially_satisfied, (), (), (),
                ["All desired predicates are already satisfied."],
            )

        result = self._search(goal, snapshot)
        status = "planned" if result.complete else "blocked" if result.blocked else "incomplete"
        ...
        if status == "blocked":
            rationale.append("No policy-compliant capability sequence can satisfy the unresolved predicates.")
        elif status == "incomplete":
            rationale.append(f"Maximum step limit reached ({goal.maximum_steps}).")
        return self._make_plan(
            goal, snapshot, status, result.steps, initially_satisfied, unresolved,
            result.decisions, result.rejected, rationale,
        )
    ...
    def _search(self, goal: DomainGoal, snapshot: DomainStateSnapshot) -> _SearchResult:
        ...
        seen: set[tuple[tuple[str, str, str, str], tuple[str, ...]]] = set()
        while queue:
            state = queue.popleft()
            ...
            for contract in candidates:
                decision = self.policies.evaluate(f"use_capability:{contract.capability_id}", state.snapshot)
                ...
                if (
                    any(step.capability_id == contract.capability_id for step in state.steps)
                    and all(self._effect_established(effect, state.snapshot) for effect in contract.effects)
                ):
                    continue

                pre = self._ensure_predicates(goal, contract.preconditions, state.snapshot, state.steps)
                if pre is None:
                ...
                satisfies = [
                    i for i, predicate in enumerate(goal.desired_state)
                ...
                rationale = (*state.rationale, *pre_rationale, f"Selected {contract.capability_id} to satisfy predicate index {target_index}.")
                post = simulated
                for validation_id in contract.validation_capability_ids:
                    validation = self.capabilities.get(validation_id)
                    validation_decision = self.policies.evaluate(f"use_capability:{validation_id}", post)
                ...
                if not added_steps:
                    continue
                signature = (self._snapshot_signature(post), tuple(step.capability_id for step in added_steps))
                if signature in seen:
                    continue
                seen.add(signature)
                expanded = True
                queue.append(_SearchResult(
                    (*state.steps, *added_steps), post, decisions, rejected, rationale, False, False
                ))
            if not expanded and candidates:
                best_incomplete = best_incomplete or _SearchResult(
                    state.steps, state.snapshot, state.decisions, state.rejected, state.rationale, False, True
                )
        if complete_results:
            winner = min(complete_results, key=self._result_key)
            return _SearchResult(
                winner.steps, winner.snapshot, winner.decisions,
                self._merge_rejections(winner.rejected, tuple(rejected_pool)),
                winner.rationale, True, False,
            )
        if best_incomplete:
            return _SearchResult(
                best_incomplete.steps, best_incomplete.snapshot, best_incomplete.decisions,
                self._merge_rejections(best_incomplete.rejected, tuple(rejected_pool)),
                best_incomplete.rationale, False, best_incomplete.blocked,
            )
        return _SearchResult((), snapshot, (), tuple(rejected_pool), (), False, True)

    def _ensure_predicates(self, goal, predicates, snapshot, existing_steps):
        if not predicates:
            return snapshot, (), (), (), ()
        ...
        for contract in self.capabilities.all():
            for effect in contract.effects:
                if effect.subject != target.subject or effect.predicate != target.predicate:
                    continue
                if PredicateOperator(target.operator) is PredicateOperator.EQUALS and effect.value != target.value:
                    continue
    ...
    @staticmethod
    def _rejection(goal, contract, decision):
        if contract.side_effect_class in goal.forbidden_side_effects:
            return RejectedCapability(
                capability_id=contract.capability_id,
                reason=f"Side-effect class {contract.side_effect_class!r} is forbidden by the goal.",
            )
    ...
    def _simulate(self, snapshot, contract, goal_id: str):
        ...
        for effect in contract.effects:
            if effect.operation == "invalidate":
                ...
                facts.append(DomainFact(
                    fact_id=f"plan-fact-{uuid4().hex}", project_id=snapshot.project_id,
                    subject=effect.subject, predicate=effect.predicate, value=effect.value,
                    status=effect.status, confidence=effect.confidence, authority=effect.authority,
                    evidence_refs=[f"plan://{goal_id}/{contract.capability_id}/{effect.predicate}"],
                    source="domain_planner_simulation", observed_at=now,
                ))
        return snapshot.with_facts(facts, revision=snapshot.revision)

    def _effect_established(self, effect, snapshot) -> bool:
        if effect.operation == "invalidate":
            return not snapshot.active_facts(effect.subject, effect.predicate)
        predicate = DomainPredicate(
            subject=effect.subject, predicate=effect.predicate,
            operator=PredicateOperator.EQUALS, value=effect.value,
            minimum_confidence=effect.confidence, minimum_authority=effect.authority,
        )
        return self.predicates.evaluate(predicate, snapshot).satisfied

    @staticmethod
    def _result_key(result: _SearchResult):
        return (
            len(result.steps),
    ...
    def _step(self, contract, indexes):
        return PlannedCapabilityStep(
            step_id=f"step-{uuid4().hex}", capability_id=contract.capability_id,
            satisfies_predicate_indexes=sorted(indexes), required_predicates=contract.preconditions,
            expected_effects=contract.effects, side_effect_class=contract.side_effect_class,
            validation_capability_ids=contract.validation_capability_ids,
        )

    @staticmethod
    def _snapshot_signature(snapshot):
        return tuple(sorted((f.subject, f.predicate, str(f.value), f.status) for f in snapshot.facts))

    @staticmethod
    def _append_decision(items, decision):
        return (*items, decision) if decision.action not in {item.action for item in items} else items

    @staticmethod
    def _append_rejection(items, rejection):
        return (*items, rejection) if rejection.capability_id not in {item.capability_id for item in items} else items

    @staticmethod
    def _merge_rejections(left, right):
        result = list(left)
        for item in right:
    ...
    @staticmethod
    def _make_plan(goal, snapshot, status, steps, initial, unresolved, decisions, rejected, rationale):
        return DomainPlan(
            plan_id=f"domain-plan-{uuid4().hex}", goal=goal, observed_revision=snapshot.revision,
            status=status, steps=list(steps), initially_satisfied_predicate_indexes=list(initial),
            unresolved_predicate_indexes=list(unresolved), policy_decisions=list(decisions),
            rejected_capabilities=list(rejected), rationale=list(rationale),
        )
